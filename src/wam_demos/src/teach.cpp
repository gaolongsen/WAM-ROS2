 ...
 * Possible args are:
    ...
    * -t <record_type>
        ...
        * "-t tv" -> record using tool velocities
        * "-t tp" -> record using tool positions
  * Option defines what record mode to use. Possible values are:
        * 0 -> record using joint positions
        * 1 -> record using joint velocities
        * 2 -> record using tool velocities
        * 3 -> record using tool positions
*/
void parseArgs(int argc, char** argv, int& option, std::string& ros_bag_name) {
  if (argc == 3) {  //either one of the args specified
    ...
    if (strcmp(flag, "-n") == 0) { //argv[2] is name
      ...
      } else if (strcmp(record_mode, "tp") == 0) {
        option = 1;
        ROS_INFO("Recording using tool poses");
      } else if (strcmp(record_mode, "tv") == 0) {
        option = 2;
        ROS_INFO("Recording using tool velocities");
      } else if (strcmp(record_mode, "jv") == 0) {
        option = 3;
        ROS_INFO("Recording using joint velocities");
      } else {
        ROS_ERROR(
            "Incorrect recording data type specified: must be jp, tp, tv or "
            "jv.");
        exit(0);
      }
    } else {
...
int main(int argc, char** argv) {
  ...
  Teach teach(
      ros_bag_name,
      n.serviceClient<std_srvs::Trigger>("/wam/idle"), option);
  ros::Subscriber wam_jp_sub =
      n.subscribe("/joint_states", 1000, &Teach::wamJointPositionCb, &teach);
  ros::Subscriber wam_jv_sub = n.subscribe(
      "/wam/jointVelocity", 1000, &Teach::wamJointVelocityCb, &teach);
  ros::Subscriber wam_tp_sub =
      n.subscribe("/wam/ToolPose", 1000, &Teach::wamToolPoseCb, &teach);
  ros::Subscriber wam_tv_sub = n.subscribe(
      "/wam/toolVelocity", 1000, &Teach::wamToolVelocityCb, &teach);
  std::string input;
